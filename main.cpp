...
void* block(size_t size)
{
	...
	//size -= SIZE_H;
	void* pointer = memory + sizeof(header_t);
...
void* mem_realloc(void* pointer, size_t size)
{
	...
	{
		if (get_size(pointer) - size - SIZE_H >= 0) // new size must be more than header size
		{
	...
	if (get_next(pointer) != nullptr && get_size(pointer) + get_size(get_next(pointer)) //if there is a next block that is size_1 + size_2 >= size
		>= size)
	{
